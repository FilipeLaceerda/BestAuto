...
import {
    Cabecalho, CentralizadoNaMesmaLinha, EntradaNomeProduto
} from '../../assets/styles';
...
export default class Feeds extends React.Component {
  ...
  carregarFeeds = () => {
    ...
    if (empresaEscolhida) {
      const maisFeeds = feedsEstaticos.feeds.filter(
        feed => feed.company._id == empresaEscolhida._id,
      );
    ...
    return (
      <CentralizadoNaMesmaLinha>
        <EntradaNomeProduto
          ...
          value={nomeProduto}></EntradaNomeProduto>
        <Icon
          style={{padding: 8}}
          size={20}
          name="search1"
    ...
    return (
      <View>
        <Header
          ...
          rightComponent={<></>}
          containerStyle={Cabecalho}></Header>
        <FlatList
          data={feeds}
          numColumns={2}
          ...
          renderItem={({item}) => {
            return <View style={{width: '50%'}}>{this.mostrarFeed(item)}</View>;
          }}></FlatList>
      </View>
    );
  };
